...
public class SpaceView extends Panel{
	...
	private void setStatistics(SpaceInformation spaceViewIndexedContainer) {
		...
		if(spaceViewIndexedContainer.numberOfDatasets > 0){
			vert.addComponent(new Label(String.format("Last Change: %s", String.format("In Sample: %s. Date: %s",spaceViewIndexedContainer.lastChangedSample.split("/")[2], spaceViewIndexedContainer.lastChangedDataset.toString()))));
		}
		vert.addComponent(this.table);
	}
